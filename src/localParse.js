...
const EXPLICIT_TIME_PATTERN = new RegExp(`(?:上午|早上|早|早晨|下午|晚上|傍晚|夜里|夜晚|夜|今晚|凌晨|中午)?${ZH_HOUR}(?:点|时|:)(?:\\d{1,2}分?|半)?`, 'g')
const TIME_RANGE_PATTERN = new RegExp(`(上午|早上|早|早晨|下午|晚上|傍晚|夜里|夜晚|夜|今晚|凌晨|中午)?\\s*(${ZH_HOUR}|\\d{1,2}:\\d{2})\\s*(点半|点\\d{1,2}分?|点|时|:\\d{2})?\\s*(?:-|—|～|~|到|至)\\s*(${ZH_HOUR}|\\d{1,2}:\\d{2})\\s*(点半|点\\d{1,2}分?|点|时|:\\d{2})`, 'g')
...
function parseTimeValue(value, suffix = '', period = '') {
  ...
  if (period && /下午|晚上|傍晚|夜里|夜晚|夜|今晚|中午/.test(period) && hour < 12) hour += 12
...
const DATE_TIME_PATTERNS = [
  ...
  /\d{1,2}月\d{1,2}[日号]/g,
  /\d{1,2}号/g,
  /(?:上午|早上|早晨|下午|晚上|傍晚|夜里|凌晨|中午|今晚)?(?:\d{1,2}|十[一二]?|两|[一二三四五六七八九])(?:点|时|:)?(?:\d{1,2}分?|半)?\s*(?:-|—|～|~|到|至)\s*(?:\d{1,2}|十[一二]?|两|[一二三四五六七八九])(?:点|时|:)(?:\d{1,2}分?|半)?/g,
