...
const ZH_MINUTE = '(?:\\d{1,2}|一刻|三刻|半|[一二三四五六七八九十两])'
const TIME_PERIOD_PATTERN = '上午|早上|明早|早|早晨|下午|晚上|傍晚|夜里|夜晚|夜|今晚|凌晨|中午'
const EXPLICIT_TIME_PATTERN = new RegExp(`(?:${TIME_PERIOD_PATTERN})?${ZH_HOUR}(?:点|时|:)(?:${ZH_MINUTE}分?|半)?`, 'g')
const TIME_RANGE_PATTERN = new RegExp(`(${TIME_PERIOD_PATTERN})?\\s*(${ZH_HOUR}|\\d{1,2}:\\d{2})\\s*(点半|点一刻|点三刻|点\\d{1,2}分?|点${ZH_MINUTE}|点|时|:\\d{2})?\\s*(?:-|—|～|~|到|至)\\s*(${ZH_HOUR}|\\d{1,2}:\\d{2})\\s*(点半|点一刻|点三刻|点\\d{1,2}分?|点${ZH_MINUTE}|点|时|:\\d{2})`, 'g')
...
function parseTimeValue(value, suffix = '', period = '') {
  ...
  if (value.includes(':')) {
    ...
    if (suffix === '点半') {
      ...
      minute = parseMinuteToken(suffix.slice(1, -1))
    } else if (suffix.startsWith(':')) {
      minute = parseInt(suffix.slice(1), 10)
    } else if (suffix.startsWith('点') && suffix.length > 1) {
      minute = parseMinuteToken(suffix.slice(1))
    }
  }

  if (Number.isNaN(hour) || Number.isNaN(minute)) return null

  if (normalizedPeriod && /下午|晚上|傍晚|夜里|夜晚|夜|今晚|中午/.test(normalizedPeriod) && hour < 12) hour += 12
  if (normalizedPeriod && /凌晨/.test(normalizedPeriod) && hour === 12) hour = 0
...
function translateToEnglish(text) {
  ...
  t = t.replace(new RegExp(`(?:上午|早上|明早|早|早晨)(${ZH_HOUR})(?:点|时|:)(\\d{2})?`, 'g'), (_, h, m) => `${parseHour(h)}:${m || '00'} am`)
...
const DATE_TIME_PATTERNS = [
  ...
  /\d{1,2}月\d{1,2}[日号]/g,
  /\d{1,2}号/g,
  /(?:上午|早上|早晨|明早|下午|晚上|傍晚|夜里|凌晨|中午|今晚)?(?:\d{1,2}|十[一二]?|两|[一二三四五六七八九])(?:点|时|:)?(?:\d{1,2}分?|一刻|三刻|半)?\s*(?:-|—|～|~|到|至)\s*(?:\d{1,2}|十[一二]?|两|[一二三四五六七八九])(?:点|时|:)(?:\d{1,2}分?|一刻|三刻|半)?/g,
  /(?:上午|早上|早晨|明早|下午|晚上|傍晚|夜里|凌晨|中午|今晚)(?:\d{1,2}|十[一二]?|两|[一二三四五六七八九])(?:点|时)(?:\d{1,2}分?|一刻|三刻|半)?/g,
  /(?:\d{1,2}|十[一二]?|两|[一二三四五六七八九])点(?:\d{1,2}分?|一刻|三刻|半)?/g,
...
export function localParse(text, todayStr) {
  ...
  const hasTimeKeyword = /点|时|上午|下午|晚上|早上|明早|今晚|到|至|-|—|~|～|一刻|三刻|半/.test(text)
